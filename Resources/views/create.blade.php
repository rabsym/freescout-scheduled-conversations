...
@section('title_full', __('Create Scheduled Conversation') . ' - ' . $mailbox->name)
...
<div class="section-heading margin-bottom">
    {{ __('Create Scheduled Conversation') }}
</div>
...
<div class="container">
    <div class="row">
        <div class="col-md-10 col-md-offset-1">
            ...
            <form method="POST" action="{{ route('scheduledconversations.store', ['mailbox_id' => $mailbox->id]) }}" id="scheduledconv-form">
                {{ csrf_field() }}
                    ...
                    <div class="controls">
                        <div class="onoffswitch-wrap">
                            <div class="onoffswitch">
                                <input type="checkbox" name="active" value="1" checked="" id="active" class="onoffswitch-checkbox">
                ...
                <div class="form-group{{ $errors->has('destination_type') ? ' has-error' : '' }}">
                    ...
                    <div class="radio">
                        <label>
                            <input type="radio" name="destination_type" value="internal" id="dest_internal" {{ old('destination_type', 'internal') === 'internal' ? 'checked' : '' }}>
                            {{ __('Internal message to this mailbox') }} <small class="text-muted">({{ __('No email sent') }})</small>
                        </label>
                    </div>
                    <div class="radio">
                        <label>
                            <input type="radio" name="destination_type" value="customer" id="dest_customer" {{ old('destination_type') === 'customer' ? 'checked' : '' }}>
                            {{ __('FreeScout customer address') }}
                        </label>
                    </div>
                    <div class="radio">
                        <label>
                            <input type="radio" name="destination_type" value="email" id="dest_email" {{ old('destination_type') === 'email' ? 'checked' : '' }}>
                            {{ __('Email Address') }}
                        </label>
                    </div>
                ...
                <div class="form-group" id="destination_value_group" style="display:none;">
                    ...
                    <div id="dest_customer_field" style="display:none;">
                        ...
                        <input type="text" id="customer_search" class="form-control"
                               ...
                               autocomplete="off"
                               data-search-url="{{ route('scheduledconversations.search_customers') }}">
                    ...
                    <div id="dest_email_field" style="display:none;">
                        ...
                        <input type="email" name="destination_email" id="destination_email" class="form-control" value="{{ old('destination_email') }}" placeholder="user@example.com">
                ...
                <div class="form-group{{ $errors->has('frequency_type') ? ' has-error' : '' }}">
                    ...
                    <select name="frequency_type" id="frequency_type" class="form-control">
                        <option value="once"            {{ old('frequency_type', 'once') === 'once'            ? 'selected' : '' }}>{{ __('Once') }}</option>
                        <option value="daily"           {{ old('frequency_type') === 'daily'           ? 'selected' : '' }}>{{ __('Daily') }}</option>
                        <option value="weekly"          {{ old('frequency_type') === 'weekly'          ? 'selected' : '' }}>{{ __('Weekly') }}</option>
                        <option value="monthly"         {{ old('frequency_type') === 'monthly'         ? 'selected' : '' }}>{{ __('Monthly') }}</option>
                        <option value="monthly_ordinal" {{ old('frequency_type') === 'monthly_ordinal' ? 'selected' : '' }}>{{ __('Monthly (nth weekday)') }}</option>
                        <option value="yearly"          {{ old('frequency_type') === 'yearly'          ? 'selected' : '' }}>{{ __('Yearly') }}</option>
                    </select>
                ...
                <div id="frequency_fields" class="well well-sm margin-top">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group{{ $errors->has('once_date') ? ' has-error' : '' }}" id="fg-once-date">
                                    ...
                                    <input type="date" name="once_date" id="once_date" class="form-control" value="{{ old('once_date') }}">
                            ...
                            <div class="col-md-6">
                                <div class="form-group{{ $errors->has('once_time') ? ' has-error' : '' }}" id="fg-once-time">
                                    ...
                                    <input type="time" name="once_time" id="once_time" class="form-control" value="{{ old('once_time', '09:00') }}">
                    ...
                    <div id="freq_daily" class="frequency-config" style="display:none;">
                        <div class="form-group{{ $errors->has('daily_time') ? ' has-error' : '' }}" id="fg-daily-time">
                            <label>{{ __('Time') }} <span class="text-danger">*</span></label>
                            <input type="time" name="daily_time" id="daily_time" class="form-control" value="{{ old('daily_time', '09:00') }}">
                    ...
                    <div id="freq_weekly" class="frequency-config" style="display:none;">
                        <div class="form-group{{ $errors->has('weekly_day') ? ' has-error' : '' }}">
                            ...
                            <select name="weekly_day" class="form-control">
                                <option value="1" {{ old('weekly_day', 1) == 1 ? 'selected' : '' }}>{{ __('Monday') }}</option>
                                <option value="2" {{ old('weekly_day') == 2 ? 'selected' : '' }}>{{ __('Tuesday') }}</option>
                                <option value="3" {{ old('weekly_day') == 3 ? 'selected' : '' }}>{{ __('Wednesday') }}</option>
                                <option value="4" {{ old('weekly_day') == 4 ? 'selected' : '' }}>{{ __('Thursday') }}</option>
                                <option value="5" {{ old('weekly_day') == 5 ? 'selected' : '' }}>{{ __('Friday') }}</option>
                                <option value="6" {{ old('weekly_day') == 6 ? 'selected' : '' }}>{{ __('Saturday') }}</option>
                                <option value="0" {{ old('weekly_day') == 0 ? 'selected' : '' }}>{{ __('Sunday') }}</option>
                            </select>
                        </div>
                        <div class="form-group{{ $errors->has('weekly_time') ? ' has-error' : '' }}">
                            <label>{{ __('Time') }} <span class="text-danger">*</span></label>
                            <input type="time" name="weekly_time" class="form-control" value="{{ old('weekly_time', '09:00') }}">
                            @if ($errors->has('weekly_time'))
                                <span class="help-block">{{ $errors->first('weekly_time') }}</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div id="freq_monthly" class="frequency-config" style="display:none;">
                        <div class="form-group{{ $errors->has('monthly_day') ? ' has-error' : '' }}">
                            ...
                            <select name="monthly_day" class="form-control">
                                @for ($d = 1; $d <= 31; $d++)
                                    <option value="{{ $d }}" {{ old('monthly_day', 1) == $d ? 'selected' : '' }}>{{ $d }}</option>
                                @endfor
                            </select>
                        </div>
                        <div class="form-group{{ $errors->has('monthly_time') ? ' has-error' : '' }}">
                            <label>{{ __('Time') }} <span class="text-danger">*</span></label>
                            <input type="time" name="monthly_time" class="form-control" value="{{ old('monthly_time', '09:00') }}">
                            @if ($errors->has('monthly_time'))
                                <span class="help-block">{{ $errors->first('monthly_time') }}</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div id="freq_monthly_ordinal" class="frequency-config" style="display:none;">
                            ...
                            <div class="col-md-6">
                                <div class="form-group{{ $errors->has('monthly_ordinal_position') ? ' has-error' : '' }}">
                                    ...
                                    <select name="monthly_ordinal_position" class="form-control">
                                        <option value="first"  {{ old('monthly_ordinal_position', 'first') == 'first'  ? 'selected' : '' }}>{{ __('First') }}</option>
                                        <option value="second" {{ old('monthly_ordinal_position') == 'second' ? 'selected' : '' }}>{{ __('Second') }}</option>
                                        <option value="third"  {{ old('monthly_ordinal_position') == 'third'  ? 'selected' : '' }}>{{ __('Third') }}</option>
                                        <option value="fourth" {{ old('monthly_ordinal_position') == 'fourth' ? 'selected' : '' }}>{{ __('Fourth') }}</option>
                                        <option value="last"   {{ old('monthly_ordinal_position') == 'last'   ? 'selected' : '' }}>{{ __('Last') }}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group{{ $errors->has('monthly_ordinal_day') ? ' has-error' : '' }}">
                                    ...
                                    <select name="monthly_ordinal_day" class="form-control">
                                        <option value="1" {{ old('monthly_ordinal_day', 1) == 1 ? 'selected' : '' }}>{{ __('Monday') }}</option>
                                        <option value="2" {{ old('monthly_ordinal_day') == 2 ? 'selected' : '' }}>{{ __('Tuesday') }}</option>
                                        <option value="3" {{ old('monthly_ordinal_day') == 3 ? 'selected' : '' }}>{{ __('Wednesday') }}</option>
                                        <option value="4" {{ old('monthly_ordinal_day') == 4 ? 'selected' : '' }}>{{ __('Thursday') }}</option>
                                        <option value="5" {{ old('monthly_ordinal_day') == 5 ? 'selected' : '' }}>{{ __('Friday') }}</option>
                                        <option value="6" {{ old('monthly_ordinal_day') == 6 ? 'selected' : '' }}>{{ __('Saturday') }}</option>
                                        <option value="0" {{ old('monthly_ordinal_day') == 0 ? 'selected' : '' }}>{{ __('Sunday') }}</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        <div class="form-group{{ $errors->has('monthly_ordinal_time') ? ' has-error' : '' }}">
                            <label>{{ __('Time') }} <span class="text-danger">*</span></label>
                            <input type="time" name="monthly_ordinal_time" class="form-control" value="{{ old('monthly_ordinal_time', '09:00') }}">
                            @if ($errors->has('monthly_ordinal_time'))
                                <span class="help-block">{{ $errors->first('monthly_ordinal_time') }}</span>
                            @endif
                        </div>
                    </div>
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group{{ $errors->has('yearly_month') ? ' has-error' : '' }}">
                                    ...
                                    <select name="yearly_month" id="yearly_month" class="form-control">
                                        @for ($m = 1; $m <= 12; $m++)
                                            <option value="{{ $m }}" {{ old('yearly_month', 1) == $m ? 'selected' : '' }}>
                            ...
                            <div class="col-md-4">
                                <div class="form-group{{ $errors->has('yearly_day') ? ' has-error' : '' }}">
                                    ...
                                    <select name="yearly_day" class="form-control" id="yearly_day">
                                        @for ($d = 1; $d <= 31; $d++)
                                            <option value="{{ $d }}" {{ old('yearly_day', 1) == $d ? 'selected' : '' }}>{{ $d }}</option>
                                        @endfor
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group{{ $errors->has('yearly_time') ? ' has-error' : '' }}">
                                    <label>{{ __('Time') }} <span class="text-danger">*</span></label>
                                    <input type="time" name="yearly_time" class="form-control" value="{{ old('yearly_time', '09:00') }}">
                                    @if ($errors->has('yearly_time'))
                                        <span class="help-block">{{ $errors->first('yearly_time') }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
                ...
                <div class="form-group" id="catch_up_mode_group">
                    ...
                    <div class="radio">
                        <label>
                            <input type="radio" name="catch_up_mode" value="skip" {{ old('catch_up_mode', 'skip') === 'skip' ? 'checked' : '' }}>
                            {{ __('Skip missed executions till next cycle') }} <small class="text-muted">({{ __('Recommended') }})</small>
                        </label>
                    </div>
                    <div class="radio">
                        <label>
                            <input type="radio" name="catch_up_mode" value="catch_up_last" {{ old('catch_up_mode') === 'catch_up_last' ? 'checked' : '' }}>
                        ...
                        <div class="form-group{{ $errors->has('body') ? ' has-error' : '' }}" id="fg-body" style="clear: both; display: block;">
                            ...
                            <div class="scheduled-editor-wrapper">
                                ...
                                @if ($isProActive)
                                    ...
                                    <textarea name="body" id="body" class="form-control js-pro-editor" rows="12">{{ old('body', '') }}</textarea>
                                @else
                                    <textarea name="body" id="body" class="form-control js-std-editor" rows="12">{{ old('body') }}</textarea>
                                @endif
                            </div>
                            @if ($errors->has('body'))
                ...
                <div class="form-group margin-top">
                    <button type="submit" class="btn btn-primary" id="scheduledconv-submit">
                        <i class="glyphicon glyphicon-ok"></i> {{ __('Create Scheduled Conversation') }}
                    </button>
    ...
    $(document).ready(function() {
        ...
        function updateCatchUpMode() {
            ...
            if (freqType === 'once') {
                ...
                $('input[name="catch_up_mode"]').prop('checked', false);
                $('input[name="catch_up_mode"][value="skip"]').prop('checked', true);
            }
        }
